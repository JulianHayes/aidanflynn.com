      ...
      <table className="w-full text-left" role="table">
        <thead>
          <tr className="border-b-2 border-stone">
            ...
            <th className="py-3 px-4 text-small font-semibold text-charcoal">Spot price per gram</th>
            <th className="py-3 px-4 text-small font-semibold text-charcoal">We pay (%)</th>
            <th className="py-3 pl-4 text-small font-semibold text-charcoal">We pay per gram</th>
          </tr>
        </thead>
        <tbody>
          {goldRows.map((row) => (
            <tr key={row.carat} className="border-b border-stone/50">
              <td className="py-4 pr-4 font-semibold text-charcoal">{row.carat}</td>
              <td className="py-4 px-4 text-warm-grey">{formatCurrency(row.spotPrice, currency)}</td>
              <td className="py-4 px-4 text-warm-grey">{Math.round(row.buyPct * 100)}%</td>
              <td className="py-4 pl-4 font-semibold text-gold">{formatCurrency(row.wePayPerGram, currency)}</td>
            </tr>
          ))}
        </tbody>
      </table>
